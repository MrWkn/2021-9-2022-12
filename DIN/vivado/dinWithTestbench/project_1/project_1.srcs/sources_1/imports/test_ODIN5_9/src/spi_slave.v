module spi_slave #(
    ...
    // Global inputs -----------------------------------------
    input  wire                 RST_async,//全局的复位信号 - RST
    // SPI slave interface ------------------------------------
    input  wire                 SCK,
    output wire                 MISO,
    input  wire                 MOSI,
    ...
    (* mark_debug = "true" *)output reg  [          1:0] CTRL_OP_CODE,
    ...
    (* mark_debug = "true" *)output reg                  SPI_UPDATE_UNMAPPED_SYN,
	...
	(* mark_debug = "true" *)reg  [19:0]    spi_shift_reg_in;
	(* mark_debug = "true" *)reg  [19:0]    spi_shift_reg_out;
    (* mark_debug = "true" *)reg  [19:0]    spi_data;
    (* mark_debug = "true" *)reg  [19:0]    spi_addr;
    genvar i;
	//----------------------------------------------------------------------------------
	//SPI circuitry
	//----------------------------------------------------------------------------------
	// SPI counter   spi_cnt、spi_addr
	always @(negedge SCK, posedge RST_async)//spi_cnt 是一个计数器，从0自增到39
	...
	always @(negedge SCK, posedge RST_async)
        if (RST_async) begin
            ...
            CTRL_READBACK_EVENT <= (spi_shift_reg_in[2*M+1:2*M] != 2'b0); //[17:16]
            // spi_shift_reg_in[19:0]={10 00 ....时=0;  10 (01/10/11) ...时=1}
            CTRL_PROG_EVENT     <= 1'b0;
            CTRL_SPI_ADDR       <= spi_shift_reg_in[2*M-1:  0];  //CTRL_SPI_ADDR <= spi_shift_reg_in[15:0]
            CTRL_OP_CODE        <= spi_shift_reg_in[2*M+1:2*M];  //CTRL_OP_CODE <= spi_shift_reg_in[17:16]    [00]
            CTRL_PROG_DATA      <= {(2*M){1'b0}};    //CTRL_PROG_DATA <= {16'b0}
		end else if (spi_shift_reg_in[18] && (spi_cnt == 6'd19)) begin  //LSB is W
            spi_shift_reg_out   <= 20'b0;
            CTRL_READBACK_EVENT <= 1'b0;
            CTRL_PROG_EVENT     <= 1'b0;
            CTRL_SPI_ADDR       <= spi_shift_reg_in[2*M-1:  0]; //CTRL_SPI_ADDR <= [15:0]   ={16'b0}
            CTRL_OP_CODE        <= spi_shift_reg_in[2*M+1:2*M]; //CTRL_OP_CODE <= [17:16]   =[00]
            CTRL_PROG_DATA      <= {(2*M){1'b0}};               //CTRL_PROG_DATA <= {16'b0}
    ...
    /* 理解：
     assign readback_weight = SYNARRAY_RDATA >> (({3'b0,CTRL_SPI_ADDR[2*M-2:2*M-3]} << 3)); //CTRL_SPI_ADDR[14:13]
     SYNARRAY_RDATA[31:0] >> { CTRL_SPI_ADDR[14:13] * 8 }  按照字节地址 CTRL_SPI_ADDR[14:13] ，取出对应地址方格内的两个突触权重（共8bit）。
    */
    ...
    always @(posedge SCK)//SPI_GATE_ACTIVITY  允许SPI访问神经元和突触存储进行编程和回读
    ...
    always @(posedge SCK)//SPI_OPEN_LOOP  防止神经元arry在本地产生的尖峰事件进入调度器，依次他们不会被控制器处理（调度器只处理从AER接口接收到的事件）
    ...
    generate
        for (i=0; i<(N>>4); i=i+1) begin  //i [0:15]
            always @(posedge SCK)//将256个神经元中的每一个配置为抑制性或兴奋性
    ...
    always @(posedge SCK)//定义增加调度程序脉冲间隔计数器所需的时钟周期（仅在使用神经元突发行为时有用，否则，该寄存器应设置为0）
    ...
    always @(posedge SCK)//定义神经元脉冲时AER输出事件的来源，（在事件被生成时，从神经元 or 从控制器？）
