module synaptic_core #(
        ...
        for (i=0; i<8; i=i+1) begin
            sdsp_update #(
                ...
                if (state == BIST)      CTRL_PRE_EN  = 8'hFF;
                      // SDSP update logic    8个突触权重是同时开始并且完成更新的，且只需要两个时钟周期
                      //  单个神经元的双稳态事件，需要更新256个突触权重（32行） ，因此需要64个时钟周期；
                      //  all的神经元双稳态事件，需要更新256*256个突触权重（32*256行），因此需要 64*CLK 
                ...
                if (state == POP_NEUR)  CTRL_PRE_EN = (ctrl_cnt[3:0] == 4'b0001) ? 8'hFF : 8'b0;
                        ！！！2.配置为1，即 SPI_UPDATE_UNMAPPED_SYN配置为1，因此syn_pre只取决于控制器中的CTRL_PRE_EN，
    ...
    generate
        for (i=0; i<4; i=i+1) begin   // i= 00 01 10 11
            assign SYNARRAY_WDATA[(i<<3)+7:(i<<3)] = SPI_GATE_ACTIVITY_sync
                                                   ?
                                                       ((i == CTRL_SPI_ADDR[14:13])   //CTRL_SPI_ADDR[14:13]代表一行的8个突触权重，4个单位
    ...
    //-----------------调用  IP  SRAM
        synapse_mem_0 synarray_0 (   //synapse_mem_0 调用IP
        // Global inputs
        .clk         (CLK),
...
//     */      
//        reg [31:0] SRAM[8191:0];   //SRAM 
//        reg [31:0] Qr;
//        always @(posedge CK) begin
//            Qr <= CS ? SRAM[A] : Qr;     //第一个时钟，CS=1，可以根据地址A读取SRAM中的突触权重w
//            if (CS & WE) SRAM[A] <= D;  //往往第二个时钟， 才可以写入突触权重数据D到内存SRAM中
//        end
//        assign Q = Qr;
//endmodule
